...
def rotate_image(image):
    ...
    angle = random.uniform(
        -5, 5
    )  # change these to modify rotation, negative is to the left
...
def warp_perspective(image):  # not sure if we want to use this, some images a bit funky
    ...
    dst_pts = np.float32(
        [  # adding the random warp
            ...
            [
                cols - random.uniform(0, max_shift_x),
                rows - random.uniform(0, max_shift_y),
            ],
        ]
    )
...
def blur_image(image):
    ...
    radius = random.uniform(
        1, 5
    )  # change these to modify blur, found through manual testing: dont go much over 5
